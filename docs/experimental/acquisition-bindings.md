...
The separate experimental [measurement bindings profile](measurement-bindings.md) adds explicit channel and nominal capture-time checks. Instrument research eligibility, dataset release/session composition, native adapters, shared CLI/browser diagnostics, independent reproduction, and publication remain outstanding. This increment publishes no packages and imports no partner data.
