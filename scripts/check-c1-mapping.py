...
roots = {'id', 'temporal', 'location', 'createdAt', 'updatedAt', 'summary', 'objectCharacteristics', 'eventType', 'movement', 'sensorEvidence', 'environment', 'aviation', 'schemaVersion', 'extensions'}
expected = {}
def pointer(ref):
    node = schema
...
with (root / 'docs/c1/baseline-mapping.csv').open() as stream:
...
assert all(r['destination'] and r['meaning'] and r['disposition'] in {'existing','reorganized','revised'} for r in rows)
print(f'{len(rows)} C1 baseline occurrences accounted for; semantic equivalence still requires review.')
