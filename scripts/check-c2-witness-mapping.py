...
print(f'{len(rows)} C2a witness/account baseline occurrences accounted for; semantic equivalence still requires review.')

assert all(r['owner'] for r in rows)
assert all(r['owner'] == 'C2b documents/digital provenance' for r in rows if r['disposition'] == 'deferred')
