...
import type { ExperimentalClaimHistory } from "@disclosureos/records/experimental/v2";
...
export function migrate(args: ParsedArgs): void {
  ...
  try {
    ...
    const records = rows.map((legacy, index) => {
      ...
      if (!reasons.length && object(legacy)) {
        ...
        if (parsed.success) candidate = parsed.data;
          ...
          reasons.push(
            ...parsed.issues.map(
              (issue) => `${issue.pointer}: ${issue.message}`
            )
          );
      }
      return {
    ...
    if (args.flags["json"]) console.log(JSON.stringify(report, null, 2));
      ...
      console.log(
        `Experimental migration dry run: ${
          rows.length
        } inputs, ${candidates} review candidates, ${
    ...
    if (candidates !== rows.length) process.exitCode = 1;
  } catch (error) {
