...
export function resolveMigration(args: ParsedArgs) {
  ...
  try {
    ...
    const plan = parsed.data,
      snapshot = verifiedMigrationLedger(resolve(args.positional[0]!));
    ...
    const inventory = new Map(
      snapshot.ledger.identities.map((identity) => [
        identity.identity,
        identity,
      ])
    );
    const decisions = new Map<
      string,
      MigrationResolution["decisions"][number]
    >();
    ...
    if (args.flags["json"]) console.log(JSON.stringify(report, null, 2));
      ...
      console.log(
        `Migration resolution: ${selected.length} selected candidates, ${report.counts.deferredRows} deferred rows, ${report.counts.unresolvedRows} unresolved rows, ${report.counts.pendingReview} pending, ${report.counts.quarantined} quarantined, 0 migrated.`
      );
