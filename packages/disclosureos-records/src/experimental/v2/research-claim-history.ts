...
export function parseResearchClaimHistory(
  ...
  const contract: ResearchClaimHistoryValidation["contract"] = {
    kind: "claim_history",
    schemaId: RESEARCH_CLAIM_HISTORY_SCHEMA_ID,
    rulesetVersion: RESEARCH_CLAIM_HISTORY_RULESET_VERSION,
  };
  const parsed = ResearchClaimHistorySchema.safeParse(input);
  if (!parsed.success)
    return {
      success: false,
      contract,
  ...
  const claims = new Map<string, ResearchHistoricalClaim>();
  ...
  const validation: ResearchClaimHistoryValidation = {
    contract,
    issues,
    uncheckedRefs: [
